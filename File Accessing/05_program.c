...
int main() {
    ...
    int *numbers = (int*)malloc(amount * sizeof(int));  // Dynamically allocate memory
    if (numbers == NULL) {
    ...
    if (choice == 'y' || choice == 'Y') {
        ...
        if (fh == NULL) {
            ...
            free(numbers);  // Free memory before exiting
            return 1;
        }

        // Read the numbers from the file into an array
        for (int i = 0; i < amount; i++) {
            fscanf(fh, "%d", &numbers[i]);  // Corrected: use &numbers[i]
        }
    ...
    fflush(stdin);
    char choice_2 = '\0';
    printf("Do you want to print it on the console? (y/n): ");
    scanf(" %c", &choice_2);  // Handle input correctly

    if (choice_2 == 'y' || choice_2 == 'Y') {
        fh = fopen("sorted_data.txt", "r");
        if (fh == NULL) {
            printf("Error opening the file for reading!\n");
            free(numbers);  // Free memory before exiting
            return 1;
        ...
        int *sorted_data = (int*)malloc(amount * sizeof(int));  // Dynamically allocate memory
        if (sorted_data == NULL) {
            printf("Memory allocation failed!\n");
            fclose(fh);
            free(numbers);  // Free memory before exiting
        ...
        for (int i = 0; i < amount; i++) {
            fscanf(fh, "%d", &sorted_data[i]);  // Corrected: use &sorted_data[i]
        }

        for (int j = 0; j < amount; j++) {
            if (j == amount - 1) {
                printf(" %d ", sorted_data[j]);  // No comma after the last element
            } else {
        ...
        fclose(fh);
        free(sorted_data);  // Free memory after usage
    }

    free(numbers);  // Free memory after usage
    return 0;
}
